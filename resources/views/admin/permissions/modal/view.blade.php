      ...
      @endphp
            @foreach($table_columns as $t)
             <tr>
                ...
                @elseif(str_contains($t, '_image') || str_contains($t, 'image')  || str_contains($t, 'picture') || str_contains($t, 'picture'))
               <td>
                    @if(!$is_multiple)
                            @if(str_contains($row->{$t},'.jpg') || str_contains($row->{$t},'.png') || str_contains($row->{$t},'.gif'))
                                @php
                            ...
                            @else
                                   @php
                                    ...
                                    @if($path)
                                    <br>
                                        <i class="bx bx-download"></i> <a href="{{$path}}" download >{{ $model->{$image_field_name} }}</a>
                                    @endif
                            @endif
                        @else  {{-- if multiple --}}
