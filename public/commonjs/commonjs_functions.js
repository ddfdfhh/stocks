    ...
    if (input.files && input.files[0]) {
      ...
      if(file['type'].search('image') >= 0)
      {
          reader.onload = function(event) {
            ...
            $("#"+placeToInsertImagePreview).append(`<img src='${y}'  class='img_rounded' />`);
    ...
    if (input.files) {
        ...
        for (i = 0; i < filesAmount; i++) {
          ...
          if(file['type'].search('image') >= 0)
          {
                ...
                reader.onload = function(event) {
                    let y=event.target.result;
                    $("#"+placeToInsertImagePreview).append(`<img src='${y}'  class='img_rounded' style='width:100px;height:100px;margin:5px' />`);
                }
...
function showDependentSelectBox(dependee_key,dependent_key,value,dependent_select_box_id,table,table_id='id',callback)
{
  if(value.length==0)
  {
    return false;
...
function showDependentSelectBoxForMultiSelect(dependee_key,dependent_key,value=[],dependent_select_box_id,table,table_id='id',callback)
{
  ...
  let obj={dependee_key,dependent_key,value:JSON.stringify(value),table,table_id};
  var callbackSuccess=function(response){
         $("select#"+dependent_select_box_id).html(response['message']);
         callback();
  }
  objectAjaxNoLoaderNoAlert(obj,'/getDependentSelectDataMultipleVal',callbackSuccess);
  
}
function dynamicAddRemoveInputBox(container_id,todo,key,input_id_to_remove=undefined)
{
  ...
  $('#'+selectboxid).autocomplete({
   source: function (request, response) {
      ...
      $.ajax(ajaxOpt).done(function (data) {
        response(data);/**==========Response(data )shold be array with item {value:'v1', label:'Value 1',extradata:'jQuery1'} ====******/
      });
    }
  })
}
function multiSelectCheckBoxAction(value,field,url,table){  /******field coulumn whose value is to be set to value **/
Swal.fire({
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          selected_users = $("#" + selectboxid).val();
            ...
            objectAjaxWithBtnAndLoader("btnid", obj, '/assignUser', callbackSuccess);
        }
    });
}
...
function passwordStrengthChecker(){
  ...
  console.log(html);
    ...
    function viewRecord(id,url,module){
      $(`#${module}_modal .modal-body`).html('<div class="spinner-border text-muted"></div>')
          let obj={
            id:id
          }
          let callbackSuccess=function(res){
              var myModal = new bootstrap.Modal(document.getElementById(`${module}_modal`), {});
              myModal.show();
              setTimeout(function(){
    ...
    function deleteFileSelf(file_name,modelName,folder_name,field_name,row_id){
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          let obj={
            file_name,modelName,folder_name,field_name,row_id
          }
        let callback=function(res){
          // location.reload();
          $('#img_div').hide();
