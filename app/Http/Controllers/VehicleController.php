...
class VehicleController extends Controller
{
     public function __construct(){
        ...
        $this->index_url=route('vehicles.index');
        $this->module='Vehicle';
        $this->view_folder='vehicles';
    ...
    [
        'column' => 'document1_image',
        'label' => 'Document1',
        'sortable' => 'No'
    ],
    [
        'column' => 'document2_image',
        'label' => 'Document 2',
        'sortable' => 'No'
    ],
    [
    ...
    [
        'field_name' => 'document1_image',
        'single' => true
    ],
    [
        'field_name' => 'document2_image',
        'single' => true
    ]
];
    ...
    [
        ...
        if ($request->ajax())
         {
            ...
            if(empty($search_by))
               $search_by='name';
            $list = Vehicle::when(!empty($search_val),function($query) use($search_val,$search_by){
        ...
        {
                $query=Vehicle::with(array_column($this->model_relations,'name'));
        }
        else{
            $query=Vehicle::query();
        }
    ...
    [
        ...
        'inputs' => [
            ...
            [
                ...
                'tag' => 'input',
                'type' => 'text',
                'default' => isset($model) ? $model->model : "",
                'attr' => []
            ],
            [
                'placeholder' => 'Enter vehicle_type',
                'name' => 'vehicle_type',
                'label' => 'Vehicle Type',
                'tag' => 'input',
                'type' => 'text',
                'default' => isset($model) ? $model->vehicle_type : "",
                'attr' => []
            ],
            [
                'placeholder' => 'Enter document1',
                'name' => 'document1_image',
                'label' => 'Document1',
                'tag' => 'input',
                'type' => 'file',
                'default' => isset($model) ? $model->document1_image : "",
                'attr' => []
            ],
            [
                'placeholder' => 'Enter document2',
                'name' => 'document2_image',
                'label' => 'Document2',
                'tag' => 'input',
                'type' => 'file',
                'default' => isset($model) ? $model->document2_image : "",
                'attr' => []
            ]
        ]
    ]
             ...
             if ($this->has_upload) {
                foreach ($this->form_image_field_name as $item) {
                    ...
                    $parent_table_field = !empty($item['parent_table_field']) ? $item['parent_table_field'] : null;
                            $this->upload($request->file($field_name), $vehicle->id, $image_model_name, $parent_table_field);
                        } else {
                            $image_name = $this->upload($request->file($field_name));
                            if ($image_name) {
                                $vehicle->{$field_name} = $image_name;
                                $vehicle->save();
                            }
                        }
 ...
 public function edit($id)
    ...
    [
        ...
        'inputs' => [
            ...
            [
                ...
                'tag' => 'input',
                'type' => 'file',
                'default' => isset($model) ? $model->document2 : "",
                'attr' => []
            ]
        ]
    ]
        ...
        array_push($data[0]['inputs'], $y);
         ...
         $view_data=[ 
             ...
             'storage_folder'=>$this->storage_folder,
              'repeating_group_inputs'=>$this->repeating_group_inputs,
              'toggable_group'=>$this->toggable_group,
             'plural_lowercase'=>'vehicles','model'=>$model
             ];
    ...
    public function show($id)
    {
        ...
        $data['row'] =null;
         if(count($this->model_relations)>0)
        {
               $data['row']=Vehicle::with(array_column($this->model_relations,'name'))->findOrFail($id);
        }
        else{
            $data['row']=Vehicle::findOrFail($id);
        }
        ...
        $data['plural_lowercase']='vehicles';
 ...
 public function upload($request_files, $parent_table_id = null, $image_model_name = null, $parent_table_field = null)
    ...
    [
     ...
     public function exportVehicle(Request $request,$type){
       $filter=[]; $filter_date=[];
        $date_field=null;
        foreach($_GET as $key=>$val){
          if(str_contains($key,'start_')){
           ...
           if($type=='excel')
        return Excel::download(new \App\Exports\VehicleExport($this->model_relations,$filter,$filter_date,$date_field),'vehicles'.date("Y-m-d H:i:s").'.xlsx',\Maatwebsite\Excel\Excel::XLSX);
        if($type=='csv')
        return Excel::download(new \App\Exports\VehicleExport($this->model_relations,$filter,$filter_date,$date_field),'vehicles'.date("Y-m-d H:i:s").'.csv',\Maatwebsite\Excel\Excel::CSV);
        if($type=='pdf')
      return Excel::download(new \App\Exports\VehicleExport($this->model_relations,$filter,$filter_date,$date_field),'vehicles'.date("Y-m-d H:i:s").'.pdf',\Maatwebsite\Excel\Excel::MPDF);
