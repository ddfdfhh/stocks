...
class UnitController extends Controller
{
     public function __construct(){
        ...
        $this->index_url=route('units.index');
        $this->module='Unit';
        $this->view_folder='units';
    ...
    [
        ...
        if ($request->ajax())
         {
            ...
            if(empty($search_by))
               $search_by='name';
            $list = Unit::when(!empty($search_val),function($query) use($search_val,$search_by){
        ...
        {
                $query=Unit::with(array_column($this->model_relations,'name'));
        }
        else{
            $query=Unit::query();
        }
             ...
             if ($this->has_upload) {
                foreach ($this->form_image_field_name as $item) {
                    ...
                    $parent_table_field = !empty($item['parent_table_field']) ? $item['parent_table_field'] : null;
                            $this->upload($request->file($field_name), $unit->id, $image_model_name, $parent_table_field);
                        } else {
                            $image_name = $this->upload($request->file($field_name));
                            if ($image_name) {
                                $unit->{$field_name} = $image_name;
                                $unit->save();
                            }
                        }
 ...
 public function edit($id)
        ...
        array_push($data[0]['inputs'], $y);
         ...
         $view_data=[ 
             ...
             'storage_folder'=>$this->storage_folder,
              'repeating_group_inputs'=>$this->repeating_group_inputs,
              'toggable_group'=>$this->toggable_group,
             'plural_lowercase'=>'units','model'=>$model
             ];
    ...
    public function show($id)
    {
        ...
        $data['row'] =null;
         if(count($this->model_relations)>0)
        {
               $data['row']=Unit::with(array_column($this->model_relations,'name'))->findOrFail($id);
        }
        else{
            $data['row']=Unit::findOrFail($id);
        }
 ...
 public function upload($request_files, $parent_table_id = null, $image_model_name = null, $parent_table_field = null)
    ...
    [
		...
		$columns=\DB::getSchemaBuilder()->getColumnListing('units');
        natcasesort($columns);
     ...
     public function exportUnit(Request $request,$type){
       $filter=[]; $filter_date=[];
        $date_field=null;
        foreach($_GET as $key=>$val){
          if(str_contains($key,'start_')){
           ...
           if($type=='excel')
        return Excel::download(new \App\Exports\UnitExport($this->model_relations,$filter,$filter_date,$date_field),'units'.date("Y-m-d H:i:s").'.xlsx',\Maatwebsite\Excel\Excel::XLSX);
        if($type=='csv')
        return Excel::download(new \App\Exports\UnitExport($this->model_relations,$filter,$filter_date,$date_field),'units'.date("Y-m-d H:i:s").'.csv',\Maatwebsite\Excel\Excel::CSV);
        if($type=='pdf')
      return Excel::download(new \App\Exports\UnitExport($this->model_relations,$filter,$filter_date,$date_field),'units'.date("Y-m-d H:i:s").'.pdf',\Maatwebsite\Excel\Excel::MPDF);
