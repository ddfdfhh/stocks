...
class CategoryController extends Controller
{
        ...
        if ($request->ajax())
         {
            ...
            if(empty($search_by))
               $search_by='name';
            $list = Category::when(!empty($search_val),function($query) use($search_val,$search_by){
             ...
             if ($this->has_upload) {
                foreach ($this->form_image_field_name as $item) {
                    ...
                    $parent_table_field = !empty($item['parent_table_field']) ? $item['parent_table_field'] : null;
                            $this->upload($request->file($field_name), $category->id, $image_model_name, $parent_table_field);
                        } else {
                            $image_name = $this->upload($request->file($field_name));
                            if ($image_name) {
                                $category->{$field_name} = $image_name;
                                $category->save();
                            }
                        }
 ...
 public function edit($id)
        ...
        'inputs' => [
         ...
         $view_data=[ 
             ...
             'storage_folder'=>$this->storage_folder,
              'repeating_group_inputs'=>$this->repeating_group_inputs,
              'toggable_group'=>$this->toggable_group,
             'plural_lowercase'=>'categories','model'=>$model
             ];
    ...
    public function show($id)
    {
        ...
        $data['plural_lowercase']='categories';
       ...
       $filter=[]; $filter_date=[];
        ...
        foreach($_GET as $key=>$val){
          if(str_contains($key,'start_')){
           ...
           if($type=='excel')
        return Excel::download(new \App\Exports\CategoryExport($this->model_relations,$filter,$filter_date,$date_field),'categories'.date("Y-m-d H:i:s").'.xlsx',\Maatwebsite\Excel\Excel::XLSX);
        if($type=='csv')
        return Excel::download(new \App\Exports\CategoryExport($this->model_relations,$filter,$filter_date,$date_field),'categories'.date("Y-m-d H:i:s").'.csv',\Maatwebsite\Excel\Excel::CSV);
        if($type=='pdf')
      return Excel::download(new \App\Exports\CategoryExport($this->model_relations,$filter,$filter_date,$date_field),'categories'.date("Y-m-d H:i:s").'.pdf',\Maatwebsite\Excel\Excel::MPDF);
