...
//dd($model->permissions->pluck('name')->toArray());
        ...
        if ($form_type == 'view') {
            ...
            array_push($data['table_columns'], ['column' => 'permissions', 'label' => 'Permission', 'sortable' => 'No']);
    ...
    public function exportRole(Request $request, $type)
    {
        ...
        if ($type == 'excel') {
            return Excel::download(new \App\Exports\RoleExport($this->model_relations, $filter, $filter_date, $date_field), 'roles' . date("Y-m-d H:i:s") . '.xlsx', \Maatwebsite\Excel\Excel::XLSX);
        }

        if ($type == 'csv') {
            return Excel::download(new \App\Exports\RoleExport($this->model_relations, $filter, $filter_date, $date_field), 'roles' . date("Y-m-d H:i:s") . '.csv', \Maatwebsite\Excel\Excel::CSV);
        }

        if ($type == 'pdf') {
            return Excel::download(new \App\Exports\RoleExport($this->model_relations, $filter, $filter_date, $date_field), 'roles' . date("Y-m-d H:i:s") . '.pdf', \Maatwebsite\Excel\Excel::MPDF);
        }
