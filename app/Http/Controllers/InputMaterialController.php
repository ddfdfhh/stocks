...
use App\Http\Requests\InputMaterialRequest;
...
class InputMaterialController extends Controller
{
     public function __construct(){
        ...
        $this->index_url=route('input_materials.index');
        $this->module='InputMaterial';
        $this->view_folder='input_materials';
    ...
    [
        ...
        if ($request->ajax())
         {
            ...
            if(empty($search_by))
               $search_by='name';
            $list = InputMaterial::when(!empty($search_val),function($query) use($search_val,$search_by){
            ...
            $data=[
                ...
                'storage_folder'=>$this->storage_folder,
                 'plural_lowercase'=>'input_materials',
                 'module'=>$this->module,
        ...
        {
                $query=InputMaterial::with(array_column($this->model_relations,'name'));
        }
        else{
    ...
    [
        ...
        'inputs' => [
            ...
            [
                ...
                'tag' => 'select',
                'type' => 'select',
                'default' => isset($model) ? formatDefaultValueForSelectEdit($model,'unit_id', true) :getList('Unit')[0]->id,
                'attr' => [],
                'custom_key_for_option' => 'name',
                'options' => getList('Unit'),
    ...
    public function store(InputMaterialRequest $request)
    {
        try{
            $post=$request->all();
           ...
           $inputmaterial = InputMaterial::create($post);
             ...
             if ($this->has_upload) {
                foreach ($this->form_image_field_name as $item) {
                    ...
                    $parent_table_field = !empty($item['parent_table_field']) ? $item['parent_table_field'] : null;
                            $this->upload($request->file($field_name), $inputmaterial->id, $image_model_name, $parent_table_field);
                        } else {
                            $image_name = $this->upload($request->file($field_name));
                            if ($image_name) {
                                $inputmaterial->{$field_name} = $image_name;
                                $inputmaterial->save();
                            }
                        }
 ...
 public function edit($id)
    {
        ...
        $model=InputMaterial::findOrFail($id);
        ...
        array_push($data[0]['inputs'], $y);
         ...
         $view_data=[ 
             ...
             'storage_folder'=>$this->storage_folder,
              'repeating_group_inputs'=>$this->repeating_group_inputs,
              'toggable_group'=>$this->toggable_group,
             'plural_lowercase'=>'input_materials','model'=>$model
             ];
    ...
    public function show($id)
    {
        ...
        $data['row'] =null;
         if(count($this->model_relations)>0)
        {
               $data['row']=InputMaterial::with(array_column($this->model_relations,'name'))->findOrFail($id);
        }
        else{
            $data['row']=InputMaterial::findOrFail($id);
        }
        ...
        $data['plural_lowercase']='input_materials';
 ...
 public function view(Request $request)
    ...
    public function update(InputMaterialRequest $request, $id)
    {
        try
        {
             $post=$request->all();
            
            $inputmaterial = InputMaterial::findOrFail($id);
 ...
 public function upload($request_files, $parent_table_id = null, $image_model_name = null, $parent_table_field = null)
    ...
    [
		...
		$columns=\DB::getSchemaBuilder()->getColumnListing('input_materials');
        natcasesort($columns);
     ...
     public function exportInputMaterial(Request $request,$type){
       $filter=[]; $filter_date=[];
        $date_field=null;
        foreach($_GET as $key=>$val){
          if(str_contains($key,'start_')){
           ...
           if($type=='excel')
        return Excel::download(new \App\Exports\InputMaterialExport($this->model_relations,$filter,$filter_date,$date_field),'input_materials'.date("Y-m-d H:i:s").'.xlsx',\Maatwebsite\Excel\Excel::XLSX);
        if($type=='csv')
        return Excel::download(new \App\Exports\InputMaterialExport($this->model_relations,$filter,$filter_date,$date_field),'input_materials'.date("Y-m-d H:i:s").'.csv',\Maatwebsite\Excel\Excel::CSV);
        if($type=='pdf')
      return Excel::download(new \App\Exports\InputMaterialExport($this->model_relations,$filter,$filter_date,$date_field),'input_materials'.date("Y-m-d H:i:s").'.pdf',\Maatwebsite\Excel\Excel::MPDF);
