...
use App\Http\Requests\TransferProductStockRequest;
...
class TransferProductStockController extends Controller
{
    public function __construct()
    {
        $this->dashboard_url = \URL::to('/admin');
        $this->index_url = route('transfer_product_stocks.index');
        $this->module = 'TransferProductStock';
        $this->view_folder = 'transfer_product_stocks';
        ...
        $this->model_relations = [
            [
                'name' => 'store',
                'class' => 'App\\Models\\TransferProductStock',
                'type' => 'BelongsTo',
            ],
            [
                'name' => 'product',
    ...
    public function index(Request $request)
    {
        ...
        if (!can('list_transfer_product_stocks')) {
        ...
        if ($request->ajax()) {
            ...
            $list = TransferProductStock::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            $data = [
                ...
                'plural_lowercase' => 'transfer_product_stocks',
                'module' => $this->module,
            ...
            $view_data = [
                ...
                'title' => 'All TransferProductStocks',
    ...
    public function view(Request $request)
    {
        ...
        $data['row'] = null;
        if (count($this->model_relations) > 0) {
            $data['row'] = TransferProductStock::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = TransferProductStock::findOrFail($id);
        }
    ...
    public function upsertStoreProductStock($post)
    {
        ...
        $update = [
            ...
            'admin_recieved_quantity' => DB::raw('admin_recieved_quantity+' . $qty),
        ];
    ...
    public function store(TransferProductStockRequest $request)
    {
        if (!can('create_transfer_product_stocks')) {
    ...
    public function edit($id)
    {
        ...
        $view_data = [
            ...
            'repeating_group_inputs' => $this->repeating_group_inputs,
            'toggable_group' => $this->toggable_group,
            'plural_lowercase' => 'transfer_product_stocks', 'model' => $model,
        ];
    ...
    {
        if (!can('view_transfer_product_stocks')) {
        ...
        $data['storage_folder'] = $this->storage_folder;
        $data['table_columns'] = $this->table_columns;
        $data['plural_lowercase'] = 'transfer_product_stocks';
        $data['module'] = $this->module;
        if ($data['is_multiple']) {
    ...
    public function update(TransferProductStockRequest $request, $id)
    {
        if (!can('edit_transfer_product_stocks')) {
            return createResponse(false, 'Dont have permission');
        }
        \DB::beginTransaction();
        try
        {
            ...
            $transferproductstock = TransferProductStock::findOrFail($id);
            ...
            $transferproductstock->update($post);
            if ($this->has_upload) {
                foreach ($this->form_image_field_name as $item) {
                    ...
                    if ($request->hasfile($field_name)) {
                        if (is_array($request->file($field_name))) {
                            ...
                            $this->upload($request->file($field_name), $transferproductstock->id, $image_model_name, $parent_table_field);
                        } else {
                            $image_name = $this->upload($request->file($field_name));
                            if ($image_name) {
                                $transferproductstock->{$field_name} = $image_name;
                                $transferproductstock->save();
                            }
                        }
    ...
    {
        if (!can('delete_transfer_product_stocks')) {
            return createResponse(false, 'Dont have permission to delete');
        }
        \DB::beginTransaction();
        try
        {
            TransferProductStock::destroy($id);
    ...
    public function exportTransferProductStock(Request $request, $type)
    {
        $filter = [];
        $filter_date = [];
        $date_field = null;
        ...
        if ($type == 'excel') {
            return Excel::download(new \App\Exports\TransferProductStockExport($this->model_relations, $filter, $filter_date, $date_field), 'transfer_product_stocks' . date("Y-m-d H:i:s") . '.xlsx', \Maatwebsite\Excel\Excel::XLSX);
        }

        if ($type == 'csv') {
            return Excel::download(new \App\Exports\TransferProductStockExport($this->model_relations, $filter, $filter_date, $date_field), 'transfer_product_stocks' . date("Y-m-d H:i:s") . '.csv', \Maatwebsite\Excel\Excel::CSV);
        }

        if ($type == 'pdf') {
            return Excel::download(new \App\Exports\TransferProductStockExport($this->model_relations, $filter, $filter_date, $date_field), 'transfer_product_stocks' . date("Y-m-d H:i:s") . '.pdf', \Maatwebsite\Excel\Excel::MPDF);
        }
