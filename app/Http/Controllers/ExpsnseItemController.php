...
use App\Http\Requests\ExpsnseItemRequest;
use App\Models\ExpsnseItem;
use File;
use \Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;
class ExpsnseItemController extends Controller
{
     public function __construct(){
        $this->dashboard_url=\URL::to('/admin');
        $this->index_url=route('spendable_items.index');
        $this->module='ExpsnseItem';
        $this->view_folder='spendable_items';
    ...
    [
        ...
        if ($request->ajax())
         {
            ...
            if(empty($search_by))
               $search_by='name';
            $list = ExpsnseItem::when(!empty($search_val),function($query) use($search_val,$search_by){
            ...
            $data=[
                ...
                'storage_folder'=>$this->storage_folder,
                 'plural_lowercase'=>'spendable_items',
                 'module'=>$this->module,
        ...
        {
                $query=ExpsnseItem::with(array_column($this->model_relations,'name'));
        }
        else{
            $query=ExpsnseItem::query();
        }
    ...
    public function store(ExpsnseItemRequest $request)
    {
        ...
        try{
           ...
           $expsnseitem = ExpsnseItem::create($post);
             ...
             if ($this->has_upload) {
                foreach ($this->form_image_field_name as $item) {
                    ...
                    $parent_table_field = !empty($item['parent_table_field']) ? $item['parent_table_field'] : null;
                            $this->upload($request->file($field_name), $expsnseitem->id, $image_model_name, $parent_table_field);
                        } else {
                            $image_name = $this->upload($request->file($field_name));
                            if ($image_name) {
                                $expsnseitem->{$field_name} = $image_name;
                                $expsnseitem->save();
                            }
                        }
 ...
 public function edit($id)
        ...
        array_push($data[0]['inputs'], $y);
         ...
         $view_data=[ 
             ...
             'storage_folder'=>$this->storage_folder,
              'repeating_group_inputs'=>$this->repeating_group_inputs,
              'toggable_group'=>$this->toggable_group,
             'plural_lowercase'=>'spendable_items','model'=>$model
             ];
    ...
    public function show($id)
    {
        ...
        $data['row'] =null;
         if(count($this->model_relations)>0)
        {
               $data['row']=ExpsnseItem::with(array_column($this->model_relations,'name'))->findOrFail($id);
        }
        else{
            $data['row']=ExpsnseItem::findOrFail($id);
        }
        ...
        $data['plural_lowercase']='spendable_items';
    ...
    public function update(ExpsnseItemRequest $request, $id)
    {
        ...
        try
        {
            ...
            $expsnseitem = ExpsnseItem::findOrFail($id);
 ...
 public function upload($request_files, $parent_table_id = null, $image_model_name = null, $parent_table_field = null)
    ...
    [
     ...
     public function exportExpsnseItem(Request $request,$type){
       $filter=[]; $filter_date=[];
        $date_field=null;
        foreach($_GET as $key=>$val){
          if(str_contains($key,'start_')){
           ...
           if($type=='excel')
        return Excel::download(new \App\Exports\ExpsnseItemExport($this->model_relations,$filter,$filter_date,$date_field),'spendable_items'.date("Y-m-d H:i:s").'.xlsx',\Maatwebsite\Excel\Excel::XLSX);
        if($type=='csv')
        return Excel::download(new \App\Exports\ExpsnseItemExport($this->model_relations,$filter,$filter_date,$date_field),'spendable_items'.date("Y-m-d H:i:s").'.csv',\Maatwebsite\Excel\Excel::CSV);
        if($type=='pdf')
      return Excel::download(new \App\Exports\ExpsnseItemExport($this->model_relations,$filter,$filter_date,$date_field),'spendable_items'.date("Y-m-d H:i:s").'.pdf',\Maatwebsite\Excel\Excel::MPDF);
